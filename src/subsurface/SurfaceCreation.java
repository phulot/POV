...
public abstract class SurfaceCreation {
	...
	SurfaceCreation(subSurface sur,int c, String meshName){
	...
	public double getPerf(){
		...
		for (int j = 0; j < 4*p.nt; j++) {
			set.add(p.V[j]);
		}
		return k/(double)set.size();
	}
	...
	public void saveTestStat(int it){
		...
		while(b){
			try {
				new FileReader(new File("data/examplesStat/"+meshName+"/"+i+".txt")).close();
				i++;
		...
		try {
			...
			theDir = new File("data/examplesStat/"+meshName+"/");
			if (!theDir.exists()) {
			    try{theDir.mkdir();} catch(SecurityException se){}
			}
			FileWriter fw = new FileWriter(new File("data/examplesStat/"+meshName+"/"+i+".txt"));
			fw.write("Mesh infos : \n");
			...
			for (int k=0;k<1;k++)//serialize tests
				writeFile(fw,it,nbiter);
			globalStat(fw);
			fw.close();
			System.out.println("similation complete\n results are printed in : "+ "data/examplesStat/"+meshName+"/"+i+".txt");
	...
	Tree createApartementTree(int c, boolean b) {
		s.tree="";
		ApartmentTree t = new ApartmentTree(c, null,s);
//		s.apartementTree(t, b);
		ApartmentTree q = t.next();
		while (q != null) {
//			s.apartementTree(q, b);
	...
	private void apartementStat(FileWriter fw) throws IOException {
		...
		while (i < p.nf) {
			...
			if (i<p.nf){
				s.TreeStat(createApartementTree(3*i,false));
				if (s.tetraType[0]<=10){
					tertatypeStat[0][19]++;
					for (int l=0;l<19;l++)
